...
import re
from tacacs_dashboard.services.locks import exclusive_lock
...
def _user_edit_lock_name(username: str) -> str:
    u = re.sub(r"[^a-zA-Z0-9_.-]+", "_", (username or "").strip())
    u = (u or "unknown")[:80]
    return f"user_edit_{u}"

def _user_edit_lock_timeout_sec() -> float:
    raw = (_read_env("USER_EDIT_LOCK_TIMEOUT_SEC", "45") or "45").strip()
    try:
        v = float(raw)
...
def _run_generate_check_restart_and_flash() -> bool:
    ...
    if result.get("queued"):
        ...
        flash("หมายเหตุ: แม้ Apply จะเข้าคิวอยู่ ระบบจะทำ Provision/Deprovision ต่อทันทีเพื่อให้ OLT ทันสถานะล่าสุด", "info")
        return True
...
def index():
    ...
    # จำกัดรายการ Device Group ที่เลือกได้ใน Users & Roles ให้ตรงกับขอบเขตของ Web Admin
    device_groups_for_ui = device_groups
    default_device_group_ids: list[str] = []
    if (not is_superadmin) and (allowed_gids is not None):
        allow = set(allowed_gids)
        device_groups_for_ui = [g for g in device_groups if (g.get('id') in allow)]
        default_device_group_ids = list(allowed_gids)
    ...
    return render_template(
        ...
        users=users,
        roles=roles,
        device_groups=device_groups_for_ui,
        default_device_group_ids=default_device_group_ids,
...
def create_user_form():
    ...
    if allowed_gids is not None:
        # Web Admin: เลือกได้เฉพาะ group ใน scope (ถ้าไม่เลือก = ทุก group ใน scope)
        if not allowed_gids:
            flash("บัญชี admin ยังไม่ได้ถูกกำหนด Device Group Scope (ให้ Superadmin กำหนดในหน้า Web Accounts)", "error")
            return redirect(url_for("users.index"))

        allow = set(allowed_gids)
        if selected:
            if not set(selected).issubset(allow):
                flash("ไม่อนุญาตให้เลือก Device Group นอกขอบเขตของบัญชีนี้", "error")
                return redirect(url_for("users.index"))
            device_group_ids = selected
        else:
            device_group_ids = list(allowed_gids)
    else:
        # Superadmin: ต้องเลือกอย่างน้อย 1 group
        if not selected:
...
def delete_user_form(username: str):
    ...
    try:
        with exclusive_lock(_user_edit_lock_name(username), timeout_sec=_user_edit_lock_timeout_sec()):
            ...
            user_target_ips = _normalize_ip_list(target.get("target_olt_ips"))
            ...
            if ok2:
                if user_target_ips:
                    # Deprovision only from the explicitly selected target OLT(s) (online only).
                    try:
                        online_set = set(_get_olt_ip_list(policy, allowed_group_ids=None))
                    except Exception:
                        online_set = set()
                    ips = [ip for ip in user_target_ips if (not online_set) or (ip in online_set)]
                    if ips:
                        _maybe_deprovision_specific_ips(username, ips)
                else:
                    # Fallback: if the user was set to "All OLTs (default)", deprovision from all
                    # currently-online OLTs in the user's device-group scope.
        ...
        flash("มีคนกำลังแก้ไข/Provision ผู้ใช้นี้อยู่ (กำลัง Apply/Provision) — กรุณาลองใหม่อีกครั้ง", "warning")
        return redirect(url_for("users.index"))
...
def edit_user_form(username):
    ...
    # จำกัดรายการ Device Group ที่เลือกได้ให้ตรงกับขอบเขตของ Web Admin
    device_groups_for_ui = device_groups
    if (not is_superadmin) and (allowed_gids is not None):
        allow = set(allowed_gids)
        device_groups_for_ui = [g for g in device_groups if (g.get('id') in allow)]
    ...
    if (not is_superadmin) and (allowed_gids is not None) and (not selected_device_group_ids):
        # Legacy/unscoped user -> default to admin scope for safer editing
        selected_device_group_ids = list(allowed_gids)
    # For Edit UI: show only online OLTs in user's scope
    choice_scope_gids = None if is_superadmin else allowed_gids
    ...
    # UI convenience: if the user currently uses "All OLTs (default)" (no explicit target list),
    # pre-select the currently-online OLTs in the selected device group(s) so the admin can
    # uncheck specific OLTs to remove/deprovision the user from them.
    if not selected_target_olt_ips and selected_device_group_ids:
        selected_target_olt_ips = _get_olt_ip_list(policy, allowed_group_ids=selected_device_group_ids)
...
def edit_user_submit(username):
    ...
    try:
        with exclusive_lock(_user_edit_lock_name(username), timeout_sec=_user_edit_lock_timeout_sec()):
            ...
            # Protect provisioning account: disallow role/status changes only.
            # Scope (device groups / target OLTs) is editable and still goes through
            # the normal superadmin/admin validation below.
            if is_prov:
                # Role
            ...
            # - admin: เลือกได้เฉพาะ group ใน scope (ถ้าไม่เลือก = คงค่าเดิม, หรือทุก group ใน scope ถ้าเดิมว่าง)
            # - superadmin: ต้องเลือกอย่างน้อย 1 group
            # - provisioning user: อนุญาตให้แก้ scope ได้ โดยยังตรวจสิทธิ์ตามปกติ
            selected_scope = _read_device_group_ids_from_form(policy)
            ...
            if allowed_gids is not None:
                ...
                allow = set(allowed_gids)
                if selected_scope:
                    if not set(selected_scope).issubset(allow):
                        flash("ไม่อนุญาตให้เลือก Device Group นอกขอบเขตของบัญชีนี้", "error")
                        return redirect(url_for("users.edit_user_form", username=username))
                    device_group_ids_to_set = selected_scope
                else:
                    device_group_ids_to_set = existing_gids or list(allowed_gids)
            else:
                device_group_ids_to_set = selected_scope
                if not device_group_ids_to_set:
                    flash("กรุณาเลือก Device Group อย่างน้อย 1 กลุ่ม", "error")
                    return redirect(url_for("users.edit_user_form", username=username))
            ...
            # NOTE: user_edit UI may pre-select all in-scope OLTs for convenience (to let the admin
            # uncheck specific OLTs). If this user was previously using "All OLTs (default)" (no explicit
            # target list) and the submitted selection equals all currently-online OLTs in scope, keep it
            # as default (empty list) to avoid unintentionally "freezing" the scope.
            if not existing_target_ips and set(target_ips) == allowed_ips:
                raw_target_ips = []
                target_ips = []
...
def zte_template_switch():
    ...
    if role not in ("superadmin", "admin"):
        flash("Emergency (zte template) ใช้ได้เฉพาะ admin/superadmin เท่านั้น", "error")
...
def zte_template_status():
    ...
    Security:
    - admin/superadmin only (same as Emergency switch)
